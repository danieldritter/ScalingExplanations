    ...
    do
        ...
        for j in "${!run_names[@]}"
            ...
            if [ "$i" -eq "$EXP_MAX_IND" ]; then
                ...
                "data_cache_dir=$6" "layer=${7}" "cascading=$8" "num_layers=${num_layers[j]}"
            else 
                srun python layer_randomization.py with "explanation_type=${explanations[i]}" "output_folder=${4}" \
...
OUTPUT_FOLDER='./dn_layer_randomization_outputs'
...
CASCADING="False"
run_explanation_set "${EXPLANATIONS[*]}" "${RUN_NAMES[*]}" "${CHECKPOINT_FOLDERS[*]}" $OUTPUT_FOLDER $NUM_EXAMPLES $CACHE_DIR $LAYER $CASCADING "${NUM_LAYERS[*]}" $SEED
CASCADING="True"
run_explanation_set "${EXPLANATIONS[*]}" "${RUN_NAMES[*]}" "${CHECKPOINT_FOLDERS[*]}" $OUTPUT_FOLDER $NUM_EXAMPLES $CACHE_DIR $LAYER $CASCADING "${NUM_LAYERS[*]}" $SEED
